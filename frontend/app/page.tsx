...
function DocumentSidebar({ sources }: { sources: Source[] }) {
  const names = [...new Set(sources.map(source => source.source))];
  return <><div className="section-label">文档工作区</div><h2>你的知识，有据可循。</h2><p>围绕 PDF 提问，从文档中寻找答案。</p><div className="document-note"><span aria-hidden="true">▤</span><h3>准备好你的文档</h3><p>当前版本通过 Streamlit 上传和管理 PDF。更新文档后，请重启问答服务以刷新索引。</p></div><div className="section-label source-label">本次检索的文档 <span>{names.length}</span></div>{names.length ? names.map(name => <div className="document" key={name}>▤ <span>{name}</span></div>) : <p className="muted">提问后，相关文档会显示在这里。</p>}<div className="sidebar-foot">基于文档回答 · 保留来源</div></>;
}
